...
--    nn = makeNetwork s1 (SNil) s1 :: SomeNeuralNetwork (ValueAndDerivative Double) 1 1
    ...
    p = Prelude.last $ Prelude.take 20 $ gradientDescent 0.05 s1 s1 nn2 e (Prelude.map (\(PList _ a) -> a) i) (NeuralNetwork2.initParams 0.5 nn2)
--    out = 
  in
    do
      ...
      print $ show $ forward s1 s1 nn2 f' (use p) (Prelude.map (\(PList _ a) -> a) i)
